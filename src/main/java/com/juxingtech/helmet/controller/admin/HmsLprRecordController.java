...
import com.juxingtech.helmet.service.IHmsLicensePlateRecognitionRecordService;
import com.juxingtech.helmet.service.IHmsLicensePlateRecognitionRecordService;
...
@RestController
@Slf4j
@RequestMapping("/licensePlateRecognitionRecords")
public class HmsLprRecordController {

    @Resource
    private IHmsLicensePlateRecognitionRecordService iHmsLicensePlateRecognitionRecordService;
    ...
    public Result list(Integer page, Integer limit, String serialNo,String startDate,String endDate) {
        LambdaQueryWrapper<HmsLicensePlateRecognitionRecord> queryWrapper = new LambdaQueryWrapper<HmsLicensePlateRecognitionRecord>()
                .like(StrUtil.isNotBlank(serialNo), HmsLicensePlateRecognitionRecord::getHelmetSerialNo, serialNo)
                ...
                .orderByDesc(HmsLicensePlateRecognitionRecord::getCreateTime);

        if (page != null && limit != null) {
            Page<HmsLicensePlateRecognitionRecord> result = iHmsLicensePlateRecognitionRecordService.page(new Page<>(page, limit), queryWrapper);
            return PageResult.success(result.getRecords(), result.getTotal());
        } else if (limit != null) {
            queryWrapper.last("LIMIT " + limit);
        }
        List<HmsLicensePlateRecognitionRecord> list = iHmsLicensePlateRecognitionRecordService.list(queryWrapper);
